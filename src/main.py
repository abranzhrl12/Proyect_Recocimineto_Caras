...
from src.face_recognition.recognition_service import RecognitionService
from src.utils.logger import get_logger
from train import train_model # Importa la función de entrenamiento
...
def main():
    ...
    model_path = './data/trained_model.xml'

    # Entrenar el modelo y obtener el mapeo de nombres
    trained_model, names = train_model(training_data_path)
    trained_model.save(model_path) # Asegúrate de guardar el modelo

    # Inicializar los módulos
    video_stream = VideoStream(camera_index=0)
    face_detector = FaceDetector(haar_cascade_path=cv2.data.haarcascades + 'haarcascade_frontalface_default.xml')
    recognition_service = RecognitionService(model_path=model_path, names=names) # Pasa el diccionario de nombres
    ...
    try:
        while True:
            ...
            for (x, y, w, h) in faces:
                ...
                name, confidence = recognition_service.recognize(roi_gray)
                print(f"Label: {name}, Confidence: {confidence}")

                if confidence < 60: # Ajusta el umbral de confianza según sea necesario
                    confidence_text = f" {100 - confidence:.2f}%"
                else:
                    name = "Desconocido"
                    confidence_text = ""
